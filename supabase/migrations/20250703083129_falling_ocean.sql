...
-- Create enum types (check if they exist first and match existing schema)
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'update_preferences_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.triggers 
    WHERE trigger_name = 'update_data_policies_updated_at'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'leads' 
    AND policyname = 'Users can insert own lead data'
  ) THEN
...
-- Get comprehensive lead statistics (using correct enum values)
